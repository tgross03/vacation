...
def objective(trial: optuna.trial.Trial):
    ...
    hyper_params = {
        "train_batch_size": int(2**(trial.suggest_int(name="train_batch_size", low=0, high=9))),
        "valid_batch_size": int(2**(trial.suggest_int(name="valid_batch_size", low=0, high=9))),
        "out_channels": [
        ...
        "optimizer": getattr(torch.optim, trial.suggest_categorical("optimizer", ["Adam", "AdamW", "NAdam", "SGD"])),
        "activation_func": getattr(torch.nn, trial.suggest_categorical("activation_func", ["PReLU", "ReLU", "LeakyReLU"])),
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(study_name="vacation", direction="maximize", storage=storage, load_if_exists=True)
