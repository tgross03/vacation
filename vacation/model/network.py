...
class Metric:
    ...
    def append(self, train_val: float | None, valid_val: float | None):
        if train_val:
            self.train_vals = torch.cat([self.train_vals,
                                         torch.Tensor([train_val])])
        if valid_val:
            self.valid_vals = torch.cat([self.valid_vals,
                                         torch.Tensor([valid_val])])
...
class VCNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = nn.Sequential(
            ...
            self._activation_func(),
            nn.Linear(in_features=self._lin_out_features[0],
                      out_features=self._lin_out_features[1]),
            self._activation_func(),
            nn.Linear(in_features=self.self._lin_out_features[1],
        ...
        def _train_epoch(self, epoch: int, train_loader: torch.utils.data.DataLoader):
            ...
            metric_vals = dict(zip(self._metrics.keys(),
                                   [torch.Tensor()] * len(self._metrics.keys())))
            ...
            for X, y in tqdm(train_loader, desc=f"Training epoch {epoch}"):
                ...
                for name, metric in self._metrics.items():
                    metric_vals[name] = torch.cat([metric_vals[name],
                                                   metric.func(y_true=y, y_pred=y_pred)])
